...
import pool from '../db';
import { hashPassword, verifyPassword, generateAccessToken, generateRefreshToken, TokenPayload } from '../auth';
...
export const login: RequestHandler = async (req, res) => {
  try {
    ...
    try {
      ...
      const payload: TokenPayload = { userId: userData.id, email: userData.email };
      const accessToken = generateAccessToken(payload);
      const refreshToken = generateRefreshToken(payload);
...
export const refresh: RequestHandler = async (req, res) => {
  try {
    ...
    const payload: TokenPayload = { userId: 'user-id', email: 'user@example.com' };
