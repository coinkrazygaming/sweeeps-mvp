    ...
    const loadData = async () => {
      ...
      try {
        if (tab === 'analytics') {
          ...
          const redemptionsList = await adminAPI.listRedemptions(accessToken, 'pending');
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900">
      <div className="max-w-7xl mx-auto px-4 py-12">
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-400 mt-2">Manage platform users, redemptions, and view analytics</p>
        </div>
        ...
        {loading ? (
          ...
          <>
            ...
            {tab === 'analytics' && analytics && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
                  <h3 className="text-slate-400 font-medium mb-2">Daily Active Users</h3>
                  <p className="text-4xl font-bold text-yellow-400">{analytics.dau}</p>
                </div>
                <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
                  <h3 className="text-slate-400 font-medium mb-2">Monthly Active Users</h3>
                  <p className="text-4xl font-bold text-yellow-400">{analytics.mau}</p>
                </div>
                <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
                  <h3 className="text-slate-400 font-medium mb-2">Total Users</h3>
                  <p className="text-4xl font-bold text-purple-400">{analytics.totalUsers}</p>
                </div>
                <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
            ...
            {tab === 'analytics' && analytics && (
              <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
                <h2 className="text-xl font-bold text-white mb-6">Currency Distribution</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div>
                    <h3 className="text-yellow-400 font-semibold mb-2">Total Gold Coins</h3>
                    <p className="text-4xl font-bold text-yellow-400">
                      {analytics.currencyDistribution.totalGoldCoins.toFixed(0)}
                    </p>
                  </div>
                  <div>
                    <h3 className="text-purple-400 font-semibold mb-2">Total Sweepstakes Coins</h3>
                    <p className="text-4xl font-bold text-purple-400">
                      {analytics.currencyDistribution.totalSweeepstakesCoins.toFixed(0)}
                    </p>
                  </div>
                </div>
              </div>
            ...
            {tab === 'redemptions' && (
              <div className="glass rounded-2xl p-8 backdrop-blur-xl border border-white/10">
                <h2 className="text-xl font-bold text-white mb-6">Pending Redemptions</h2>

                {redemptions.length === 0 ? (
                  <p className="text-slate-400 text-center py-8">No pending redemptions</p>
                ) : (
                    ...
                    {redemptions.map((redemption) => (
                      ...
                      >
                        <div className="flex-1">
                          <h3 className="text-white font-semibold">{redemption.username}</h3>
                          <p className="text-sm text-slate-400">{redemption.email}</p>
                          <div className="mt-2 flex gap-6">
                            <span className="text-yellow-400">
                              <strong>{redemption.sweepstakesCoins.toFixed(2)} SC</strong>
                            </span>
                          ...
                          {redemption.prizeDescription && (
                            <p className="text-sm text-slate-500 mt-2">{redemption.prizeDescription}</p>
                          )}
                        </div>
