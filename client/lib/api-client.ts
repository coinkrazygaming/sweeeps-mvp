...
export const redemptionsAPI = {
  createRedemptionRequest: (token: string, sweepstakesCoins: number, prizeDescription?: string) =>
    apiCall('POST', '/redemptions', { sweepstakesCoins, prizeDescription }, token),
  getRedemptionStatus: (token: string, redemptionId: string) =>
    apiCall('GET', `/redemptions/${redemptionId}`, undefined, token),
  getUserRedemptions: (token: string, status?: string, limit = 20, offset = 0) => {
